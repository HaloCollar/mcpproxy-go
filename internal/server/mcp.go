...
func (p *MCPProxyServer) handleListUpstreams(ctx context.Context) (*mcp.CallToolResult, error) {
	...
	for i, server := range servers {
		...
		// Runtime.GetAllServers and the REST /api/v1/servers path) instead of
		// leaving healthInput.OAuthStatus at its zero value (""), which
		// CalculateHealth reads as "never authenticated" and reports as
		// "Authentication required" — even once the server is Connected/Ready
		// with tools listed. This clears a stale auth-required verdict left by
		// an earlier failed/pending probe as soon as a later connection
		// succeeds and persists a fresh token; ApplyToHealthInput also
		// ignores a stale LastError while the server is Connected.
		if server.OAuth != nil {
			var storageManager *storage.Manager
			if p.mainServer != nil && p.mainServer.runtime != nil {
				storageManager = p.mainServer.runtime.StorageManager()
			}
			oauth.ApplyToHealthInput(&healthInput, storageManager, server.Name, server.URL, true)
		}
